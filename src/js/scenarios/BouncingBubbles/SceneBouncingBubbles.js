...
export default class SceneBouncingBubbles extends Scene2D {
    ...
    // ajouter une bubble dans une scène à un endroit défini
    addBubble(x, y, vx = null, vy = null) {
        // push new bubble(context,x,y,radius)
        let maNewBulle_;
        if (y > 0) {
            maNewBulle_ = new Bubble(this.context, x, 0, this.params.radius)
        } else {
            maNewBulle_ = new Bubble(this.context, x, this.height, this.params.radius)
        }
        // if (!!vx) maNewBulle.vx = 
        if (!!vx) maNewBulle_.vx = vx
        // if (!!vy) maNewBulle.vy = 
        if (!!vy) maNewBulle_.vy = vy
